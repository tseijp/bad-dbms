...
export const database = (schemaOrConfig?: DatabaseConfig | Record<string, Table>, maybeConfig?: DatabaseConfig | Record<string, Table>) => {
        ...
        const dispatch = (plan: PhysicalOp): unknown => (cfg.execute ? cfg.execute(plan) : backend ? backend.execute(plan) : [])
        const rowsOf = async (plan: PhysicalOp): Promise<Row[]> => {
                const r = await Promise.resolve(dispatch(plan))
                return (Array.isArray(r) ? (r as Row[]) : []).map(stripRid)
        }
        const run: RunFn = async (ast) => {
                if (ast.op === 'Select') return rowsOf(planSelect(ast, ctx).plan)
                const table = tableNameOf(ast.table)
                if (ast.op === 'Insert') {
                        const values = ast.values ?? []
                        if (!cfg.execute && backend) {
                                const rids = backend.catalog.insertRows(table, values)
                                return ast.returning ? rids : { rowCount: rids.length }
                        }
                        return dispatch({ op: 'Insert', table, values, returning: !!ast.returning })
                }
                const predicate = ast.where ? compilePredicate(ast.where, ctx) : () => true
                const plan: PhysicalOp = ast.op === 'Update' ? { op: 'Update', table, predicate, setters: compileSetters(ast.set, ctx), returning: !!ast.returning } : { op: 'Delete', table, predicate, returning: !!ast.returning }
                const rows = await rowsOf(plan)
                if (ast.returning) return rows
                return rows[0] ?? { rowCount: 0 }
        }
        const select = (ast: SelectAst, b: () => any) => {
        ...
        const buildTx = () => ({
                select: (f?: Columns | Record<string, SQL>) => builder(run, { op: 'Select', projection: projectionOf(f) } as SelectAst, select),
                selectDistinct: (f?: Columns | Record<string, SQL>) => builder(run, { op: 'Select', projection: projectionOf(f), distinct: true } as SelectAst, select),
                insert: (t: Table) =>
                        builder(run, { op: 'Insert', table: t } as InsertAst, (ast, b) => ({
                ...
                update: (t: Table) =>
                        builder(run, { op: 'Update', table: t } as UpdateAst, (ast, b) => ({
                ...
                delete: (t: Table) =>
                        builder(run, { op: 'Delete', table: t } as DeleteAst, (ast, b) => ({
                                where: (c?: SQL) => (c && (ast.where = c), b()),
                                returning: () => ((ast.returning = true), b()),
                        })),
        })
        type Tx = ReturnType<typeof buildTx> & { rollback(): never; transaction<T>(fn: (tx: Tx) => Promise<T> | T): Promise<T> }
        const runScope = async <T>(fn: (tx: Tx) => Promise<T> | T): Promise<T> => {
                const snap = backend ? backend.catalog.snapshot() : null
                try {
                        return await Promise.resolve(fn(txHandle()))
                } catch (e) {
        ...
        const tickRunner = (fn: (tx: Tx, c?: unknown) => unknown) => ({
                async run(extra?: unknown) {
                        ...
                        for (const row of rows) {
                                ctx.current = row
                                await Promise.resolve(fn(txHandle(), proxy))
                        }
                        ctx.current = null
                        return extra
                },
        ...
        function transaction(fn: (tx: Tx, c: unknown) => unknown): ReturnType<typeof tickRunner>
