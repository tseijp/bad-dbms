...
export const inArray = <T>(col: SQL<T>, values: ReadonlyArray<Operand<T>>): Bool => make({ type: 'binop', op: 'in', args: [col, make({ type: 'list', items: values.map(wrap) })] })
...
export const like = (col: SQL, value: Operand<string>): Bool => binop('like', col, value)
export const notLike = (col: SQL, value: Operand<string>): Bool => not(like(col, value))
export const ilike = (col: SQL, value: Operand<string>): Bool => binop('ilike', col, value)
