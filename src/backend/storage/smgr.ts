...
const fidOf = (relId: number, forkId: number) => `${relId}_${FORKS[forkId] ?? forkId}`
const keyOf = (fid: string, blockNo: number) => `${fid}/${blockNo}`
const blockOf = (key: string) => +key.slice(key.lastIndexOf('/') + 1)
const newRel = (relId: number): SMgrRelation => ({ relId, forks: FORKS.map((_, i) => ({ fid: fidOf(relId, i), nBlocks: 0, known: false })) })
...
export const createStorageManager = ({ file, pageSize = 4096 }: StorageManagerOptions): StorageManager => {
        ...
        const _open = (relId: number): SMgrRelation => {
        ...
        return {
                ...
                async read(rel, forkId, blockNo) {
                        const bytes = await file.get(keyOf(_fork(rel, forkId).fid, blockNo))
                ...
                async truncate(rel, forkId, blockNo) {
                        ...
                        for (let b = blockNo; b < fork.nBlocks; b++) dels.push(file.delete(keyOf(fork.fid, b)))
