import { NONE, bound } from './nbtree-page'
import type { Cmp, Key, LeafItem, Meta, Node, Rid } from './nbtree-page'

export const createNbtreeScans = <K extends Key>(meta: Meta, get: (id: number) => Node<K>, seek: (key: K) => { node: Node<K>; path: Node<K>[] }, cmp: Cmp<K>) => {
        const forward = (start: K, end: K, emit: (key: K, rid: Rid) => boolean): void => {
        ...
        const backward = (start: K, end: K, emit: (key: K, rid: Rid) => boolean): void => {
                if (meta.root === NONE) return
                let { node } = seek(start)
                let at = bound(node.items, start, cmp)
