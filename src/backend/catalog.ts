...
import { buildColumn, needsIndex, makeCodec, encodeCell, decodeCell, resolveInsertValue, ridKey, storageRelOf, COLUMN_FORK_BASE, INDEX_FORK_BASE } from './column'
...
export const createCatalog = ({ buffer, smgr, fsm }: CatalogDeps) => {
        ...
        const _alter = createAlterOps({ buffer, smgr, fsm })
        let _nextRelId = 1
        ...
        return {
                ...
                rename(name: string, to: string): void {
                        if (_relations.has(to)) throw new Error(`relation already exists: ${to}`)
