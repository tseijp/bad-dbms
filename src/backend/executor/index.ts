...
import { makeSeqScan, makeIndexScan, makeFilter, makeProjection } from './scan'
...
const build = (catalog: Catalog, ast: PhysicalOp): RowIterator => {
        ...
        if (ast.op === 'NestedLoopJoin') return makeNestedLoopJoin(build(catalog, ast.left), build(catalog, ast.right), ast.predicate, ast.kind)
