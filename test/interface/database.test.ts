...
import { integer, text, float } from '../../src/interface/column'
...
describe('select chain', () => {
        ...
        it('filters via where clause to the matching row only', async () => {
                ...
                const rows = await db.select().from(users).where(eq((users as any).id, 2))
...
describe('update chain', () => {
        it('reports updated count and mutates the matched score', async () => {
                ...
                const res: any = await db.update(users).set({ score: 99 }).where(eq((users as any).id, 1))
