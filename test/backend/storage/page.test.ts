...
describe('page', () => {
        it('returns an initial header where every documented field is readable', () => {
                ...
                expect(page.getHeader()).toEqual({ kind: 'data', level: 0, flags: 0, prevPageId: 0, nextPageId: 0, highKey: 0, slotCount: 0, tombstoneOffset: 0, valueOffset: 0, valueSize: 0 })
        })
        it('updates only the fields named in setHeader and leaves the rest unchanged', () => {
                ...
                expect(h).toEqual({ kind: 'leaf', level: 0, flags: 0, prevPageId: 0, nextPageId: 0, highKey: 0, slotCount: 5, tombstoneOffset: 0, valueOffset: 0, valueSize: 12 })
        })
