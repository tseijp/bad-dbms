...
describe('join projection picks columns from both tables', () => {
        ...
        it('computes an expression valuesOf spanning both joined tables', async () => {
                ...
                const result = await db
                        .select({ userId: users.id, combined: users.score.add(posts.score) })
                        .from(users)
        ...
        it('groups projected fields under a nested table object', async () => {
                const { db, users, posts } = await seedUsersPosts()
                const result = await db
                        .select({ userId: users.id, post: { id: posts.id, score: posts.score } })
                        .from(users)
                        .innerJoin(posts, eq(posts.userId, users.id))
        ...
        it.each([
                ['userId only', (u: any, _pu: any) => ({ userId: u.id }), ['userId']],
                ['postId only', (_u, p) => ({ postId: p.id }), ['postId']],
                ['both ids', (u, p) => ({ userId: u.id, postId: p.id }), ['postId', 'userId']],
                ['name and score', (u, p) => ({ name: u.name, postScore: p.score }), ['name', 'postScore']],
                ['three columns', (u, p) => ({ a: u.id, b: u.name, c: p.score }), ['a', 'b', 'c']],
                ['expression valuesOf', (u, p) => ({ sum: u.score.add(p.score) }), ['sum']],
        ...
        it.each([
                ['userId only', (u: any, _pu: any) => ({ userId: u.id })],
                ['both ids', (u, p) => ({ userId: u.id, postId: p.id })],
                ['name and score', (u, p) => ({ name: u.name, postScore: p.score })],
                ['expression valuesOf', (u, p) => ({ sum: u.score.add(p.score) })],
