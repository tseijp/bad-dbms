...
describe('chaining multiple joins across three tables', () => {
        it('joins users to posts to tags in one chained query', async () => {
                ...
                const result = await innerJoin(
                        innerJoin(
                                db.select({ userId: users.id, postId: posts.id, tagId: tags.id }).from(users),
                                posts,
                                eq(posts.userId, users.id)
                        ),
                        tags,
                        eq(tags.postId, posts.id)
                )
        ...
        it('projects columns from all three chained tables', async () => {
                ...
                const result = await innerJoin(
                        innerJoin(
                                db
                                        .select({ name: users.name, postScore: posts.score, label: tags.label })
                                        .from(users),
                                posts,
                                eq(posts.userId, users.id)
                        ),
        ...
        it('reads the matched row of a three-table chain end to end', async () => {
                ...
                const result = await innerJoin(
                        innerJoin(
                                db
                                        .select({ userId: users.id, postId: posts.id, label: tags.label })
                                        .from(users),
                                posts,
                                eq(posts.userId, users.id)
                        ),
        ...
        it('drops rows of an inner chain when the third table has no match', async () => {
                ...
                const result = await innerJoin(
                        innerJoin(
                                db.select({ userId: users.id, tagId: tags.id }).from(users),
                                posts,
                                eq(posts.userId, users.id)
                        ),
                        tags,
        ...
        it('null-fills the tag id on a chained leftJoin row whose post has no tag', async () => {
                ...
                const result = await leftJoin(
                        innerJoin(
                                db.select({ postId: posts.id, tagId: tags.id }).from(users),
                                posts,
                                eq(posts.userId, users.id)
                        ),
                        tags,
        ...
        it.each([
                ...
                ['one tag per post', [[1, 1, 1], [2, 2, 2], [3, 3, 3], [4, 4, 4]] as Array<[number, number, number]>, 4],
                ['two tags on one post', [[1, 1, 1], [2, 1, 2]] as Array<[number, number, number]>, 2],
                ['three tags on one post', [[1, 1, 1], [2, 1, 2], [3, 1, 3]] as Array<[number, number, number]>, 3],
                ['mixed hit and miss', [[1, 1, 1], [2, 9, 2], [3, 2, 3]] as Array<[number, number, number]>, 2],
                ['all tags miss', [[1, 7, 1], [2, 8, 2], [3, 9, 3]] as Array<[number, number, number]>, 0],
        ...
        it('filters a chained join on the third table column', async () => {
                ...
                const result = await innerJoin(
                        innerJoin(
                                db.select({ userId: users.id, label: tags.label }).from(users),
                                posts,
                                eq(posts.userId, users.id)
                        ),
                        tags,
                        eq(tags.postId, posts.id)
                ).where(gt(tags.label, 200))
                expect(rowsOf(result)).toHaveLength(2)
        })
