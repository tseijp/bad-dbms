...
import { rowsOf, column, innerJoin, seedNodeChain, seedPair } from './helpers'
...
describe('self join pairs a table with itself', () => {
        ...
        it('matches child 2 to parent 1 in the node chain', async () => {
                const { db, nodes } = await seedNodeChain()
                const result = await innerJoin(
                        db.select({ child: nodes.id, parentId: nodes.parentId }).from(nodes),
                        nodes,
                        eq(nodes.parentId, nodes.id)
                )
                expect(rowsOf(result).map((row) => row.child).slice().sort()).toEqual([2, 3])
        })

        it('produces no self-join row for the root node', async () => {
                const { db, nodes } = await seedNodeChain()
                const result = await innerJoin(
                        db.select({ child: nodes.id }).from(nodes),
                        nodes,
                        eq(nodes.parentId, nodes.id)
                )
                expect(column(result, 'child').includes(1)).toBe(false)
        })
        ...
        it.each([
                ['three-row chain', [[1, 0, 10], [2, 1, 20], [3, 2, 30]] as Array<[number, number, number]>, 2],
                ['flat fan to root', [[1, 0, 10], [2, 1, 20], [3, 1, 30], [4, 1, 40]] as Array<[number, number, number]>, 3],
                ['no parents', [[1, 0, 10], [2, 0, 20], [3, 0, 30]] as Array<[number, number, number]>, 0],
                ['single child', [[1, 0, 10], [2, 1, 20]] as Array<[number, number, number]>, 1],
                ['dangling parent ids', [[1, 9, 10], [2, 9, 20]] as Array<[number, number, number]>, 0],
        ])('self-joins the %s into the right pair count', async (_label, rows, expected) => {
                const { db, r } = await seedPair([], rows)
                const result = await innerJoin(
                        db.select({ child: r.id, parentId: r.fk }).from(r),
                        r,
                        eq(r.fk, r.id)
                )
