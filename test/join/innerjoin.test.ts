...
describe('innerJoin keeps only matched pairs', () => {
        it('joins every post to its owning user', async () => {
                ...
                const result = await innerJoin(
                        db.select({ userId: users.id, postId: posts.id }).from(users),
                        posts,
        ...
        it('keeps only the high-score posts after an inner join with where', async () => {
                const { db, users, posts } = await seedUsersPosts()
                const result = await innerJoin(
                        db.select({ userId: users.id, postScore: posts.score }).from(users),
                        posts,
                        eq(posts.userId, users.id)
                ).where(gt(posts.score, 6))
                expect(rowsOf(result)).toHaveLength(2)
        })
        ...
        it.each([
                ['one-to-one', [[1, 10], [2, 20]] as Array<[number, number]>, [[1, 1, 100], [2, 2, 200]] as Array<[number, number, number]>, 2],
                ['one-to-many', [[1, 10]] as Array<[number, number]>, [[1, 1, 100], [2, 1, 200], [3, 1, 300]] as Array<[number, number, number]>, 3],
                ['half matched', [[1, 10], [2, 20]] as Array<[number, number]>, [[1, 1, 100]] as Array<[number, number, number]>, 1],
                ['none matched', [[1, 10]] as Array<[number, number]>, [[1, 9, 100]] as Array<[number, number, number]>, 0],
                ['many-to-one', [[1, 10], [2, 10], [3, 10]] as Array<[number, number]>, [[1, 1, 100]] as Array<[number, number, number]>, 1],
        ...
        it.each([
                ...
                ['all three match once', [[1, 1, 1], [2, 2, 2], [3, 3, 3]] as Array<[number, number, number]>, 3],
                ['two on one left', [[1, 2, 1], [2, 2, 2]] as Array<[number, number, number]>, 2],
                ['three on one left', [[1, 1, 1], [2, 1, 2], [3, 1, 3]] as Array<[number, number, number]>, 3],
                ['mixed hit and miss', [[1, 1, 1], [2, 9, 2], [3, 2, 3]] as Array<[number, number, number]>, 2],
                ['all miss', [[1, 7, 1], [2, 8, 2], [3, 9, 3]] as Array<[number, number, number]>, 0],
                ['four rows two match', [[1, 1, 1], [2, 1, 2], [3, 9, 3], [4, 9, 4]] as Array<[number, number, number]>, 2],
                ['heavy fan-out', [[1, 2, 1], [2, 2, 2], [3, 2, 3], [4, 2, 4]] as Array<[number, number, number]>, 4],
