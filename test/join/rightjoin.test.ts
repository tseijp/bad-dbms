...
import { rowsOf, rightJoin, seedPair } from './helpers'
...
describe('rightJoin keeps every right row', () => {
        ...
        it('null-fills the left column for the unmatched right row', async () => {
                const { db, l, r } = await seedPair(
                        [[1, 10]],
                        [
                                [1, 1, 100],
        ...
        it.each([
                ...
                ['two matched', [[1, 1, 1], [2, 2, 2]] as Array<[number, number, number]>, 2],
                ['two orphan', [[1, 8, 1], [2, 9, 2]] as Array<[number, number, number]>, 2],
                ['three mixed', [[1, 1, 1], [2, 9, 2], [3, 2, 3]] as Array<[number, number, number]>, 3],
                ['fan-out then orphan', [[1, 1, 1], [2, 1, 2], [3, 9, 3]] as Array<[number, number, number]>, 3],
