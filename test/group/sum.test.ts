...
describe('per-group sum', () => {
        ...
        it('sums each post group score independently as strings', async () => {
                ...
                expect([
                        groupWith(result, 'userId', 1).s,
                        groupWith(result, 'userId', 2).s,
                        groupWith(result, 'userId', 3).s,
                ]).toEqual(['12', '9', '4'])
        })

        it.each([
                ['two even groups', [[0, 10], [0, 20], [1, 30], [1, 40]] as Array<[number, number]>, 0, '30'],
                ['negatives in a group', [[0, -5], [0, -15], [1, 100]] as Array<[number, number]>, 0, '-20'],
                ['mixed signs', [[5, -10], [5, 10], [5, 50]] as Array<[number, number]>, 5, '50'],
                ...
                const result = await db.select({ g: t.g, s: sum(t.v) }).from(t).groupBy(t.g)
