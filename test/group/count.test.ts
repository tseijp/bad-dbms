...
describe('per-group count', () => {
        it('counts two rows in the kind-0 event group', async () => {
                ...
                const result = await db
                        ...
                        .from(events)
                        .groupBy(events.kind)
                expect(groupWith(result, 'kind', 0).n).toBe(2)
        })
        ...
        it('counts one row in the kind-2 event group', async () => {
                ...
                const result = await db
                        ...
                        .from(events)
                        .groupBy(events.kind)
                expect(groupWith(result, 'kind', 2).n).toBe(1)
        })
        ...
        it.each([
                ...
                expect(groupWith(result, 'userId', userId).n).toBe(expected)
        })
        ...
        it('sums the per-group event counts back to the whole table size', async () => {
                ...
                const result = await db
                        ...
                        .from(events)
                        .groupBy(events.kind)
                expect(rowsOf(result).reduce((acc, r) => acc + r.n, 0)).toBe(5)
        })

        it.each([
                ['singletons', [[1, 0], [2, 0], [3, 0]] as Array<[number, number]>, 1, 1],
                ['one pair one single', [[1, 0], [1, 0], [2, 0]] as Array<[number, number]>, 1, 2],
                ['big and small', [[7, 0], [7, 0], [7, 0], [9, 0]] as Array<[number, number]>, 7, 3],
        ...
        it('counts distinct values inside each group', async () => {
                ...
                const result = await db
                        ...
                        .from(t)
                        .groupBy(t.g)
                expect([groupWith(result, 'g', 0).d, groupWith(result, 'g', 1).d]).toEqual([2, 1])
        })
        ...
        it('seeds events, reads per-group counts, then re-reads after a delete', async () => {
                ...
                const after = await db
                        ...
                        .from(events)
                        .groupBy(events.kind)
                expect([groupWith(before, 'kind', 0).n, groupWith(after, 'kind', 0).n]).toEqual([2, 1])
        })
