...
import { EVENTS_SEED, POSTS_SEED, findBy, makeEvents, makePosts, rowsOf, seedEvents, seedPosts } from '../_helpers'
...
describe('grouping over an insert-and-mutate usecase', () => {
        it('seeds posts, groups by owner, then re-groups after inserting a post', async () => {
                ...
                expect([findBy(before, 'userId', 2)!.n, findBy(after, 'userId', 2)!.n]).toEqual([1, 2])
        })
        ...
        it('builds groups incrementally and watches a group count climb', async () => {
                ...
                expect([findBy(one, 'kind', 7)!.n, findBy(two, 'kind', 7)!.n]).toEqual([1, 2])
        })
        it('updates a row into a different group and watches both group sizes shift', async () => {
                ...
                expect([findBy(result, 'kind', 1)!.n, rowsOf(result).length]).toEqual([3, 2])
        })
