...
describe('having filters groups by aggregate', () => {
        it('keeps only groups whose count exceeds one', async () => {
                ...
                const result = await (db
                        ...
                        .from(events)
                        .groupBy(events.kind) as any).having(gt(count(), 1))
        ...
        it('keeps the single group whose count equals one', async () => {
                ...
                const result = await (db
                        .select({ kind: events.kind, n: count() })
                        .from(events)
                        .groupBy(events.kind) as any).having(eq(count(), 1))
        ...
        it('keeps groups whose per-group sum exceeds a threshold', async () => {
                ...
                const result = await (db
                        ...
                        .from(events)
                        .groupBy(events.kind) as any).having(gt(sum(events.v), 400))
                expect(rowsOf(result)).toHaveLength(2)
        })
        ...
        it('returns an empty array when having matches no group', async () => {
                ...
                const result = await (db
                        .select({ kind: events.kind, n: count() })
                        .from(events)
                        .groupBy(events.kind) as any).having(gt(count(), 99))
        ...
        it.each([
                ...
                const result = await (db
                        .select({ kind: events.kind, n: count() })
                        .from(events)
                        .groupBy(events.kind) as any).having(predicate(count()))
        ...
        it.each([
                ...
                const result = await (db
                        .select({ kind: events.kind, s: sum(events.v) })
                        .from(events)
                        .groupBy(events.kind) as any).having(predicate(sum(events.v)))
                expect(rowsOf(result)).toHaveLength(expected)
        })
})
