...
index は user の migration ではなく column 制約 (`primaryKey` / `unique`) から catalog が自動配置。
...
src/backend/            catalog + executor + entry
  adapter/              file adapter 群 (memory / nodejs / browser / ...)
...
下位は上位の意味論を持たない。
catalog は storage path を `relId` に変換し、access 以下は `(relId, forkId, blockNo)` と `rid = [pageId, offset]` で会話。
...
catalog の `insertRows(name, rows)` は column 順に heap を呼び出し、
...
smgr   ←─── 「storage path を relation handle に変換」
            intern(path) ───→ relId
...
─────────────────────────────────────────────────────────────
primaryKey / unique  nbtree         <col>_idx
それ以外             index 無し
```
...
catalog の `insertRows(name, rows)` は column 順に各 heap へ `insert(value)` を発行。
...
```sql
relation path = <table>
column path   = <table>/<column>
index path    = <table>/<index>
例: relation  = cells_1
     columns  = [
         { name: 'id' },    path cells_1/id
         { name: 'A' },     path cells_1/A
         { name: 'B' },     path cells_1/B
     ]
     indexes  = [
         { column: 'id' },  path cells_1/id_idx
         { column: 'A' },   path cells_1/A_idx
     ]
```
`relId` は `smgr.intern(path)` が返す in-memory handle で、永続 file 名ではない。
column / index ごとに独立した page 列を持ち、block は `<path>/<blockNo>` に保存する。
